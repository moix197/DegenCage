...
**Decision:** Wallet history comes from a single Helius JSON-RPC method,
`getTransactionsForAddress` (`apps/web/src/server/chain/helius-client.ts`), and a trade is
...
- Fails closed: `chain.helius` off, or any request error, **throws**. It must never resolve
  to `[]`, which reads as "wallet has no trades" instead of "we could not check".
