...
- **The `@/*` path alias (`apps/web/tsconfig.json`) is mirrored in `vitest.config.ts`'s
  `resolve.alias`.** Vite does not read tsconfig `paths` on its own — confirmed empirically
  early in Phase 9 (`import … from '@/server/flags/feature-flags'` in a test file failed to
  resolve, "Cannot find package"), which briefly forced the phase's first two testable routes
  onto relative imports, diverging from every other route in the codebase. A two-line
  `resolve: { alias: { '@': …/apps/web/src } }` (no `vite-tsconfig-paths` dependency needed
  for a single alias) fixed the root cause instead, and those routes were reverted back to
  `@/…`. **Every route/page keeps using `@/…` uniformly** — the alias now resolves the same
  way under `next build` and under Vitest, so there is no reason for a tested file to import
  differently from an untested one.
