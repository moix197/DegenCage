...
| `@degencage/rules` | The rule engine — pure, I/O-free, the product IP; owns the constitution document schema, `evaluateTrade()`, and the exact-decimal USD helpers | `packages/rules` | [monorepo-package-shape](decisions/monorepo-package-shape.md), [constitution-schema](decisions/constitution-schema.md) |
| db | Drizzle schema, migrations, and the one pooled Postgres handle (`getDb()`). Tables: `users`, `wallets`, `siws_challenges`, `sessions`, `events`, `feature_flags`, `constitutions`, `trades`, `token_prices` | `apps/web/src/server/db` | [migration-and-test-tooling](decisions/migration-and-test-tooling.md), [single-source-of-truth-database](decisions/single-source-of-truth-database.md) |
...
| rules (server) | The I/O half of rule evaluation: rolling-window trade queries feeding `packages/rules`' pure `evaluateTrade()`, and the status page's live allowance | `apps/web/src/server/rules` | [server-side-rule-evaluation](decisions/server-side-rule-evaluation.md), [usd-pricing-source](decisions/usd-pricing-source.md) |
...
| Reconciliation safety | Idempotent by `(wallet_id, signature)` + `ON CONFLICT DO NOTHING`; per-batch row lock with external pricing kept outside the transaction; cursor advanced only via `GREATEST` to a fully-persisted slot; `baseline_completed_at` is the sole "backfill finished" signal | [reconciliation-idempotency](decisions/reconciliation-idempotency.md) |
