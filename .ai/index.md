...
| chain | Wallet history ingestion: Helius pull → swap derivation from balance deltas → `reconcileWallet()`, the pipeline that writes `trades`. Triggered on app open via `POST /api/wallet/reconcile`; no worker, no cron | `apps/web/src/server/chain` | [chain-data-source](decisions/chain-data-source.md), [reconciliation-idempotency](decisions/reconciliation-idempotency.md) |
| pricing | A swap's USD value: stablecoin leg at face, SOL leg from the shared `token_prices` minute cache; unpriceable is `null`, never `0` | `apps/web/src/server/pricing` | [usd-pricing-source](decisions/usd-pricing-source.md) |
