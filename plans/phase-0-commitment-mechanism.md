...
- **Deferred on-chain verification (accepted risk, decided 2026-08-27).** No phase in this plan is verified against a real user-initiated swap. Phase 0 never builds a swap UI, so a live pass would mean trading on Jupiter directly and reading the result back out — an out-of-product loop even after Phase 7's dashboard lands. The consolidated pass for Phases 5 and 6 is therefore deferred to roadmap Phase 1 (Jupiter terminal), where the swap can be initiated locally. **What stays unexercised until then:** live Jupiter Tokens v2 and Birdeye response shapes (a provider schema drift would not surface), the classification and long-tail pricing paths against real mints, and FIFO lot-matching against real chain-ordered activity. **Mitigation:** exhaustive unit coverage on `lot-matching.ts`, `classify-token.ts`, `price-trade.ts` and `evaluate.ts`; every provider behind a kill switch that fails closed; Phase 10's end-to-end pass exercises everything except the swap itself, using seeded trade rows. **Treat the first live swap at roadmap Phase 1 as a verification milestone, not a formality** — an unexpected provider payload there is a finding against this plan, not a test-setup problem.
...
- [x] `pnpm test` passes
- [~] Manual (**deferred out of Phase 0 — see roadmap Phase 1**): activate a constitution with a tight `MICRO_CAP` limit, execute a small real swap into a low-cap token on-chain, confirm it is flagged with correct reasoning; confirm selling out of an over-limit tier is never itself flagged; confirm flipping `classification.jupiter_mcap` off degrades every classification to `MICRO_CAP`/`unknown` rather than erroring.

  **Why deferred:** Phase 0 never builds a swap UI — executing a swap means leaving the product and trading on Jupiter directly, then waiting for reconcile. Phase 7's dashboard makes the *outcome* observable locally, but it does not make the *swap* local, so a real-swap pass at Phase 7 would still be a manual out-of-product round-trip. The consolidated on-chain pass is therefore deferred past this plan entirely, to roadmap Phase 1 (Jupiter terminal), where a swap can be initiated in our own UI and the whole loop is exercisable locally. See "Deferred on-chain verification" under Dependencies & Risks for the accepted risk.
...
- [x] `pnpm test` passes
- [~] Manual (**deferred out of Phase 0 — see roadmap Phase 1**): activate a loss-limit constitution, execute a real small round-trip at a loss on-chain, confirm it's counted; confirm a pre-existing position closed at a loss is *not* counted and the UI explains why.

  **Why deferred:** same reason as Phase 5's, and the same destination — roadmap Phase 1, not Phase 7. A live round-trip requires initiating a swap, which Phase 0 never builds; the dashboard only makes the result readable. Mitigated in the meantime by the unit tests scoped to `lot-matching.ts`, which cover the FIFO and activation-boundary rules exhaustively. See "Deferred on-chain verification" under Dependencies & Risks.
...
| modify | `apps/web/src/app/dashboard/page.tsx` | **(corrected)** calls `applyDuePendingChanges()` on app open — this is the real reconciliation entry point; `/api/wallet/reconcile` has no caller |
...
- `constitution_pending_changes` gained a `voided_at` column: a pending increase whose `old_value` no longer matches the limit's current value is **voided, not applied** (emitting `constitution.limit_increase_voided`). Without this, requesting 100→500 then decreasing to 10 would silently jump to 500 48h later — a hole in the commitment mechanism.
...
- [x] Changes committed: `feat: asymmetric constitution edits — instant decrease, 48h delayed increase`
...
- [ ] Feature tested manually end-to-end on a real Solana wallet, covering: golden path, an early-activation bypass attempt, a wallet-account-switch mid-session, a Helius outage (kill switch flipped off) resulting in a fail-closed "not reconciled" state rather than a false-clean dashboard. **Without live swaps** — see "Deferred on-chain verification" under Dependencies & Risks. Trade-dependent behavior (tier classification, FIFO lot-matching, the mixed pre/post-activation round-trip exclusion) is verified against seeded/synthetic trade rows and the unit suites, not real on-chain activity; the live pass runs at roadmap Phase 1.
...
| `decisions/deferred-on-chain-verification.md` | create | New decision (2026-08-27): no phase in Phase 0 is verified against a real user-initiated swap; the consolidated on-chain pass for Phases 5 and 6 moves to roadmap Phase 1 (Jupiter terminal), because Phase 0 never builds a swap UI and Phase 7's dashboard makes outcomes readable without making the swap local. Records what stays unexercised (live Jupiter Tokens v2 / Birdeye payload shapes, classification and long-tail pricing against real mints, FIFO matching against real chain ordering), the mitigations, and that the first live swap is a verification milestone against this plan |
